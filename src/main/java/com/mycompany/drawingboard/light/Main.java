...
public class Main {
    ...
    public static TyrusServerContainer getTyrusContainer(HttpServer server) throws DeploymentException {
        ...
        container.addEndpoint(SSEWebSocket.class);
        return container;
    }
     ...
     * @throws IOException
     */
    public static void main(String[] args) throws IOException, DeploymentException, NumberFormatException {
        final HttpServer jcontainer = getJerseyContainer();
        TyrusServerContainer tcontainer = getTyrusContainer(jcontainer);
        
        tcontainer.start("/", Integer.parseInt(port.orElse("8080")));
        System.out.println(String.format("drawingboard-light-sse started at %s"  + 
